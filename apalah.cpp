...
// Konversi nama ke huruf kapital
...
void insertUser(string id, string nama, float t, float h, int aq, bool s, float n){
	...
	User *baru = new User;
	baru->id = id;
	baru->nama = toUpper(nama); // Konversi ke kapital
	baru->temperature = t;
	baru->humidity = h;
	baru->airQuality = aq;
	baru->smoke = s;
...
// =============================================
//         REQUIREMENT 3: UPDATE USER
// =============================================
void updateUser(string id, string nama, float t, float h, int aq, bool s, float n){
	if(head == NULL){
		cout << "Data kosong.\n";
		return;
	}
	...
	while(temp != NULL){
		if(temp->id == id){
			...
			// Update data
			temp->nama = toUpper(nama);
			temp->temperature = t;
			temp->humidity = h;
			temp->airQuality = aq;
			temp->smoke = s;
			temp->noise = n;
			temp->score = hitungScore(t, h, aq, s, n);
...
void deleteUser(string id){
	...
	while(curr != NULL){
		if(curr->id == id){
			...
			delete curr;
			cout << "User dengan ID " << id << " berhasil dihapus dari sistem.\n";
			return;
		}
		prev = curr;
		curr = curr->next;
...
int main(){
	...
	do {
		...
		if(pilihan == 1){
			...
			cout << "Humidity     : "; cin >> h;
			cout << "Air Quality  : "; cin >> aq;
			cout << "Smoke (0/1)  : "; cin >> s;
			cout << "Noise        : "; cin >> n;
